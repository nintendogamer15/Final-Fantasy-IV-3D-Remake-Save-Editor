...
work_dir="$(mktemp -d /tmp/ffiv3d-arch-package.XXXXXXXX)"
trap 'rm -rf "${work_dir}"' EXIT
mkdir -p "${output_dir}"

install -m 0644 "${repo_dir}/packaging/arch/PKGBUILD" "${work_dir}/PKGBUILD"
install -m 0755 "${binary}" "${work_dir}/app-binary"
install -m 0644 "${repo_dir}/packaging/linux/ffiv3d-save-editor.desktop" "${work_dir}/ffiv3d-save-editor.desktop"
install -m 0644 "${repo_dir}/packaging/linux/io.github.nintendogamer15.FFIV3DSaveEditor.metainfo.xml" \
  "${work_dir}/io.github.nintendogamer15.FFIV3DSaveEditor.metainfo.xml"
...
for file in LICENSE COPYING COPYING.LESSER ADDITIONAL_PERMISSIONS.md THIRD_PARTY_NOTICES.md README_LICENSE_SUMMARY.txt; do
  install -m 0644 "${repo_dir}/${file}" "${work_dir}/${file}"
done
install -m 0644 "${repo_dir}/licenses/FFIV-Save-Editor-LGPL-3.0.txt" \
  "${work_dir}/FFIV-Save-Editor-LGPL-3.0.txt"
install -m 0644 "${repo_dir}/licenses/MIT.txt" "${work_dir}/MIT.txt"
...
package="${output_dir}/ffiv3d-save-editor-${version}-1-x86_64.pkg.tar.zst"
